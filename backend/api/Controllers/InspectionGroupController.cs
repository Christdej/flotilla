        ...
        )
        {
            ...
            try
            {
                ...
                var existingPlant = await plantService.ReadByInstallationAndPlantCode(
                    ...
                    inspectionGroup.PlantCode,
                    readOnly: true
                );
                if (existingPlant == null)
                {
                    return NotFound($"Could not find plant with name {inspectionGroup.PlantCode}");
                }
                var existingInspectionGroup =
                    await inspectionGroupService.ReadByInstallationAndPlantAndName(
                        existingInstallation,
                        existingPlant,
